...
Every assumption about missing data -- what it means, what the code does, which
way it bends, and whether that direction was tested or merely assumed -- is
catalogued in [`missing_data.md`](missing_data.md), regenerated by
`17_missing_data_taxonomy.py` from the data itself so it cannot drift. Eleven
entry points: four tested, four conservative by construction, three with the
