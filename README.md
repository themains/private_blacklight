...
pipeline in `scripts/R/`. Data collection stays in Python and is not re-run.

```bash
make analysis             # every table and figure (~4 min, cold)
...
| `03_import` | Blacklight and WhoTracksMe scans → domain measures |
...
| `07_demography` | Tables 5–6, coefficient plots, birth-year curve |
...
Every stochastic step takes an explicit seed, so repeated runs agree. Bootstrap
intervals and the placebo therefore reproduce exactly; they differ in their last
digits from the numbers the earlier Python pipeline published, which drew from a
different generator.
